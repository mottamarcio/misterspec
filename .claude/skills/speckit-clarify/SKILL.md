...
8. **Re-validate Spec Quality Checklist** (if it exists):
   ...
   - If it exists:
     ...
     2. Identify all checkbox lines — `- [ ]`, `- [x]`, or `- [X]` (case-insensitive, tolerant of leading whitespace for nested items) outside of code fences. Ignore all other content (headings, notes, non-checkbox bullets, metadata).
     3. For each checkbox line, record its current marker state and item text into a before-snapshot list.
     ...
     7. Compare the before-snapshot with the current state to compute three lists for the Completion Report: **Newly passing** (unchecked → checked), **Regressions** (checked → unchecked), **Still unchecked**.

9. Report completion (after questioning loop ends or early termination):
   - Number of questions asked & answered.
   - Path to updated spec.
   - Sections touched (list names).
   - Spec quality checklist status (if `FEATURE_DIR/checklists/requirements.md` was re-validated in step 8): show before/after pass counts (e.g., "Spec Quality Checklist: 12/16 → 15/16 items passing") and list any items that changed state — both newly passing and any regressions. If any items remain unchecked, list them as areas needing attention.
