...
argument-hint: "Optional focus area for the convergence check"
...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    After emitting the block above you MUST actually invoke the hook and wait for it to finish before continuing. Run it the same way you would run the command yourself in this agent/session (the invocation may differ from the literal `{command}` id shown above, e.g. it may run as a Skill/tool invocation rather than a literal slash command). Emitting the block alone does not run the hook.
...
`/speckit-implement` can complete it. This command should run only after
`/speckit-implement` has run on the current `tasks.md`, and after `/speckit-tasks` has
...
Run `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS — this already fails with a clear message naming `/speckit-plan` or `/speckit-tasks` if `plan.md` or `tasks.md` is missing. Derive absolute paths:
...
Additionally check that SPEC exists (the script itself only validates `plan.md`/`tasks.md`); if it does not, STOP with a clear, actionable message naming `/speckit-specify` as the prerequisite to run. Do not produce partial output.
...
|----|----------|----------|--------|----------|----------------|
| F1 | missing  | HIGH     | FR-008 | Example: no append-only guard detected in path/to/module.go when writing tasks.md | Add append-only enforcement |
...
**Check for extension hooks (after convergence)**:
...
- [ ] Codebase assessed against spec, plan, tasks, and constitution
- [ ] Either "✅ Converged" reported with `tasks.md` byte-for-byte unchanged, or a single new `## Phase N: Convergence` section appended with zero modification to `spec.md`, `plan.md`, or any pre-existing task
- [ ] Extension hooks dispatched or skipped according to the rules in Pre-/Post-Execution Checks above
- [ ] Completion reported to user with the outcome and next-action recommendation
