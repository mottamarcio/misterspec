...
func TestCodex_Metadata(t *testing.T) {
	a := codex.New()
	if a.ID() != "codex" {
		t.Errorf("ID() = %q, want %q", a.ID(), "codex")
	}
	if a.Name() == "" {
		t.Error("Name() is empty, want a human-readable name")
	}
	if a.TargetPath() != ".agents/skills" {
		t.Errorf("TargetPath() = %q, want %q", a.TargetPath(), ".agents/skills")
	}
}

func TestCodex_InstallFreshDirectory(t *testing.T) {
	root := t.TempDir()
	a := codex.New()
	...
	if result.AdapterID != "codex" {
		t.Errorf("result.AdapterID = %q, want %q", result.AdapterID, "codex")
	}
	if result.IntegrationPath != ".agents/skills" {
		t.Errorf("result.IntegrationPath = %q, want %q", result.IntegrationPath, ".agents/skills")
	}
	...
	for _, o := range result.Outcomes {
		...
		got, err := os.ReadFile(filepath.Join(root, ".agents", "skills", o.Resource.Name))
		if err != nil {
	...
	if record.Agent.ID != "codex" || record.Agent.IntegrationPath != ".agents/skills" {
		t.Errorf("record.Agent = %+v, unexpected", record.Agent)
	}
}

func TestCodex_InstallWithoutOverwriteSkipsAlreadyPresent(t *testing.T) {
	root := t.TempDir()
	a := codex.New()
