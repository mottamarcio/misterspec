...
func TestDevin_Metadata(t *testing.T) {
	a := devin.New()
	if a.ID() != "devin" {
		t.Errorf("ID() = %q, want %q", a.ID(), "devin")
	}
	if a.Name() == "" {
		t.Error("Name() is empty, want a human-readable name")
	}
	if a.TargetPath() != ".devin/skills" {
		t.Errorf("TargetPath() = %q, want %q", a.TargetPath(), ".devin/skills")
	}
}

func TestDevin_InstallFreshDirectory(t *testing.T) {
	root := t.TempDir()
	a := devin.New()
	...
	if result.AdapterID != "devin" {
		t.Errorf("result.AdapterID = %q, want %q", result.AdapterID, "devin")
	}
	if result.IntegrationPath != ".devin/skills" {
		t.Errorf("result.IntegrationPath = %q, want %q", result.IntegrationPath, ".devin/skills")
	}
	...
	for _, o := range result.Outcomes {
		...
		got, err := os.ReadFile(filepath.Join(root, ".devin", "skills", o.Resource.Name))
		if err != nil {
	...
	if record.Agent.ID != "devin" || record.Agent.IntegrationPath != ".devin/skills" {
		t.Errorf("record.Agent = %+v, unexpected", record.Agent)
	}
}

func TestDevin_InstallWithoutOverwriteSkipsAlreadyPresent(t *testing.T) {
	root := t.TempDir()
	a := devin.New()
