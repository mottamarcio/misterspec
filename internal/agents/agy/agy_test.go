...
func TestAgy_Metadata(t *testing.T) {
	a := agy.New()
	if a.ID() != "agy" {
		t.Errorf("ID() = %q, want %q", a.ID(), "agy")
	}
	if a.Name() == "" {
...
func TestAgy_InstallFreshDirectory(t *testing.T) {
	root := t.TempDir()
	a := agy.New()
	...
	if result.AdapterID != "agy" {
		t.Errorf("result.AdapterID = %q, want %q", result.AdapterID, "agy")
	}
	...
	if record.Agent.ID != "agy" || record.Agent.IntegrationPath != ".agents/skills" {
		t.Errorf("record.Agent = %+v, unexpected", record.Agent)
	}
}

func TestAgy_InstallWithoutOverwriteSkipsAlreadyPresent(t *testing.T) {
	root := t.TempDir()
	a := agy.New()
