...
func TestCursorAgent_Metadata(t *testing.T) {
	a := cursoragent.New()
	if a.ID() != "cursor-agent" {
		t.Errorf("ID() = %q, want %q", a.ID(), "cursor-agent")
	}
	if a.Name() == "" {
		t.Error("Name() is empty, want a human-readable name")
	}
	if a.TargetPath() != ".cursor/skills" {
		t.Errorf("TargetPath() = %q, want %q", a.TargetPath(), ".cursor/skills")
	}
}

func TestCursorAgent_InstallFreshDirectory(t *testing.T) {
	root := t.TempDir()
	a := cursoragent.New()
	...
	if result.AdapterID != "cursor-agent" {
		t.Errorf("result.AdapterID = %q, want %q", result.AdapterID, "cursor-agent")
	}
	if result.IntegrationPath != ".cursor/skills" {
		t.Errorf("result.IntegrationPath = %q, want %q", result.IntegrationPath, ".cursor/skills")
	}
	...
	for _, o := range result.Outcomes {
		...
		got, err := os.ReadFile(filepath.Join(root, ".cursor", "skills", o.Resource.Name))
		if err != nil {
	...
	if record.Agent.ID != "cursor-agent" || record.Agent.IntegrationPath != ".cursor/skills" {
		t.Errorf("record.Agent = %+v, unexpected", record.Agent)
	}
}

func TestCursorAgent_InstallWithoutOverwriteSkipsAlreadyPresent(t *testing.T) {
	root := t.TempDir()
	a := cursoragent.New()
